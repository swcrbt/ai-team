...
- Primary product surfaces: chat command center, team/model/role/member management, project workspace and patch review, command approval, task history, audit/settings.
...
## Product goals
- Goals: help a user coordinate model-backed team work, supervise task queues, approve sensitive commands, review patches, and inspect audit history without losing context.
- Non-goals: social chat polish, consumer onboarding, autonomous file writes, or hiding safety state behind settings.
- Success signals: the active task, member state, pending command, proposed patch, and audit trail are visible from the relevant work surface.

## Personas and jobs
- Primary personas: local developer/operator, team-orchestration power user, and reviewer of model-generated file changes.
- User jobs: configure models/roles/members, start team or member chats, monitor serial/parallel collaboration, approve commands, apply/reject diffs, inspect history and audit details.
...
- Primary navigation: compact dark icon rail for chat, teams, models, roles, members, history, audit, project, and settings.
- Core routes/screens: Chat Command Center, Team Setup and Routing, Project Safety and Patch Review, Settings Audit Console.
- Content hierarchy: active work first, safety confirmation second, historical/audit evidence third.

## Design principles
- Principle 1: keep command, patch, queue, and audit state close to the action that created it.
- Principle 2: prefer dense, scannable panels with stable dimensions over decorative containers.
- Tradeoffs: a right-side task/audit inspector adds density but reduces message width; keep it fixed and collapsible if implementation needs smaller windows.
...
- Imagery/iconography: functional icons only, with tooltip support in implementation.

## Components
- Existing components to reuse: Flutter Material 3 widgets, existing sidebars, chat pane, split send button, task queue cards, shared dialog frame, management panels, audit/history rows.
- New/changed components: right-side task inspector, safety-oriented project review panel, audit detail drawer, management category sidebar, compact state pills.
- Variants and states: pending/running/paused/completed/failed tasks, command pending/approved/executed/denied/failed, patch pending/applied/rejected, model streaming/stopped/failed, empty workspace.
...
- Layout adaptations: icon rail stays fixed, conversation/management sidebars can shrink or collapse, right inspector becomes a drawer below constrained widths.
...
- Empty: no workspace, no audit, no history, and no command request states should provide one clear primary action.
...
- Test/screenshot expectations: implementation should add widget coverage for navigation, queue/approval states, and run `flutter test`, `flutter analyze`, and `flutter build macos --debug` before completion.

## Open questions
- [ ] Should implementation adopt the new persistent right-side task inspector, or keep the existing bottom task expansion for the first pass?
...
- [ ] Should settings and audit remain separate navigation items, or merge visually while keeping route compatibility?
